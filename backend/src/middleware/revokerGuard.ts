import { Request, Response, NextFunction } from "express";
import { publicClient } from "../services/arcService.js";
import { ADDRESSES } from "../config/arc.js";
import { ATTESTATION_REGISTRY_ABI } from "../abis/AttestationRegistry.js";

export async function revokerGuard(req: Request, res: Response, next: NextFunction) {
  const caller = req.verifiedAddress;
  if (!caller) {
    res.status(401).json({
      success: false,
      error: { code: "NO_ADDRESS", message: "No verified address" },
    });
    return;
  }

  if (!ADDRESSES.attestationRegistry) {
    res.status(500).json({
      success: false,
      error: { code: "NO_CONTRACT", message: "AttestationRegistry not configured" },
    });
    return;
  }

  try {
    const REVOKER_ROLE = (await publicClient.readContract({
      address: ADDRESSES.attestationRegistry,
      abi: ATTESTATION_REGISTRY_ABI,
      functionName: "REVOKER_ROLE",
    })) as `0x${string}`;

    const hasRole = await publicClient.readContract({
      address: ADDRESSES.attestationRegistry,
      abi: ATTESTATION_REGISTRY_ABI,
      functionName: "hasRole",
      args: [REVOKER_ROLE, caller],
    });

    if (!hasRole) {
      res.status(403).json({
        success: false,
        error: { code: "NOT_REVOKER", message: `${caller} does not hold REVOKER_ROLE` },
      });
      return;
    }
  } catch (err) {
    res.status(502).json({
      success: false,
      error: { code: "CHAIN_ERROR", message: "Failed to check revoker role onchain" },
    });
    return;
  }

  next();
}
