...
interface PassportView {
  address:     string;
  credentials: { claimId: string; schemaId: string; issuer: string; valid: boolean }[];
  services:    Record<ServiceKey, { claimCount: number; verified: boolean }>;
}

export function CredentialViewPage() {
  ...
  const credentialServices: ServiceKey[] = ["credentials", "education"];

  return (
    <div className="py-12 px-4 animate-fade-in max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Credentials</h1>
      <p className="text-xs text-gray-500 dark:text-gray-400 font-mono mb-6">{data.address}</p>

      <div className="space-y-3">
        {credentialServices.map((key) => {
          const svc = data.services?.[key];
          if (!svc) return null;
          return (
            <div key={key} className="bg-white dark:bg-gray-800 rounded-xl shadow p-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-semibold text-gray-900 dark:text-white">{SERVICE_LABELS[key]}</p>
                <ServiceBadge name={key} verified={svc.verified} claimCount={svc.claimCount} />
              </div>
              {svc.verified ? (
                <p className="text-xs text-green-600 dark:text-green-400">
                  {svc.claimCount} active credential{svc.claimCount === 1 ? "" : "s"}
                </p>
              ) : (
                <p className="text-xs text-gray-500 dark:text-gray-400 italic">No credentials issued</p>
              )}
            </div>
          );
        })}
