...
export function DaoVerifyPage() {
  ...
  const verify = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/v1/dao/member/${input}`));
  ...
  return (
    <div className="py-12 px-4 animate-fade-in max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">DAO & Governance</h1>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Check a wallet's DAO membership and governance participation status.</p>
      ...
      {result && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-3">
          ...
          <p className="text-sm font-semibold text-gray-900 dark:text-white">DAO Status: {result.verified ? "Member" : "Not Found"}</p>
          <p className="text-xs text-gray-500 dark:text-gray-400">{result.claimCount} claim(s)</p>
          <div className="flex flex-wrap gap-2">
            {ALL_SERVICE_KEYS.map((key) => {
              const isActive = key === "dao";
